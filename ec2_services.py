...
async def text_to_speech(audio_bytes):
    from groq import Groq

    print("Transcribing audio...")
    print(f"Bytes received: {len(audio_bytes)}")
    print("Calling Groq Whisper...")

    client = Groq(api_key=os.getenv("GROQ_API_KEY"))

    try:
        # Create a file-like object from bytes and ensure it's at the start
...
async def fetch_media(media_id: str):
    ...
    # Step 1: Get the media URL (metadata)
    async with httpx.AsyncClient() as client:
        meta_res = await client.get(url, params=params)

    if meta_res.status_code != 200:
        print("MEDIA METADATA ERROR:", meta_res.text)
        return None
    ...
    if file_res.status_code != 200:
        print("MEDIA DOWNLOAD ERROR:", file_res.text)
        return None
